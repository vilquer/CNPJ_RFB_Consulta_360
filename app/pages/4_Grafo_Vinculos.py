...
with st.sidebar:
    ...
    niveis = st.slider(
        ...
        value=1,
        help=(
            "1 = sócios e administradores da empresa. "
            "2 = empresas ligadas a esses sócios. "
            "3 = sócios dessas empresas. O processo continua alternando."
        ),
    )
...
# ---------------------------------------------------------------------------
# Entrada e validação do CNPJ
# ---------------------------------------------------------------------------

entrada = st.text_input(
    "CNPJ, com 14 ou 8 dígitos, com ou sem pontuação",
    placeholder="92.702.067/0001-96",
)

if not entrada:
    st.info(
        "Digite um CNPJ. A partir dele o grafo expande sócios, "
        "administradores, representantes e empresas relacionadas."
    )
    st.stop()
...
if len(digitos) < 8:
    st.error("O CNPJ precisa ter pelo menos 8 dígitos.")
    st.stop()
